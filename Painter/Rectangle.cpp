...
{
	//Create a pen for object and initialize it
	CPen aPen;
	CreatePen(aPen);
	...
	pDC->SelectObject(pOldPen);		//Restore the old brush
	pDC->SelectObject(pOldBrush);	//Restore the old pen
}

CRectangle::CRectangle(const CPoint & start, const CPoint & end, COLORREF color) :
	CElement{ start, color }
{
	//Normalize the retangle defining points
